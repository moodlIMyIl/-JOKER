...
file:close()
sleep(1)
os.execute(' screen -S Lightning ./Lightning')
end
...
---------------------------------------- 
local tmLightning = function(msg)     
...
------------------------------------------------
local LightningEN = function(msg)     
            if is_leaderid(msg.sender_user_id_) then
                   Lightning1  = "SUDO BOT 🍃"
            elseif is_sudoid(msg.sender_user_id_) then
                    Lightning1 = "SUDO ✔️"
            elseif is_admin(msg.sender_user_id_) then      
                    Lightning1 = "ADMIN  IN BOT ♣️"
            elseif is_owner(msg.sender_user_id_, msg.chat_id_) then
                    Lightning1 = "OWNER 🎶"
            elseif is_momod(msg.sender_user_id_, msg.chat_id_) then
                Lightning1 = "ADMIN IN GP 🎵"
            elseif is_vipmem(msg.sender_user_id_, msg.chat_id_) then
                   Lightning1 = "VIP MEMBER 🔇"
            else
                  Lightning1 = "MEMBER ONLY 🔕"
            end
return Lightning1
end
--------------------------------------------
...
--------------------------------------------		 
		...
		local get_me = function(extra, result)
            ...
            if database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "🌀║ <b>your name</b> :\n<code> " .. name .. "</code>\n💲║ <b>Your Rank</b> :\n <code>" .. LightningEN(msg) .. "</code>", 1, "html")
            else
              send(msg.chat_id_, msg.id_, 1, "🌀║ <b>اســمك</b> :\n<code> " .. name .. "</code>\n💲║ <b>رتبتـك </b> :\n <code>" .. tmLightning(msg) .. "</code>" , 1, "html")
            end
          end
		  getUser(msg.sender_user_id_, get_me)
         end		 
...
‏
		...
		if text:match("^السورس$") or text:match("^مطور السورس$") or text:match("^سورس$") or  text:match("^سورس كيبر$") or text:match("^اريد سورس$") then
            if not database:get('lock:add'..msg.chat_id_) then
   local text =  [[
  ✪
⚡️║آهلا بك في سورس لايتنك الجديد...🍃

➖➖➖➖➖ •⊱⚡️⊰• ➖➖➖➖➖
`git clone https://github.com/LightningTAME/Lightning.git && cd Lightning && chmod +x Lightning && ./Lightning`
——————————————
✔️┐ طريقة تنصيب السورس....
⚀┤ فقط أضغط على الكود ليتم نسخه.
⚁┤ ثم الصق بالترمنال وانتر وانتظر يتنصب.
⚂┤ بعدها يطلب منك تخلي المعلومات منك.
⚃┘ بعد كل معلومه انتر.
——————————————
①┐ آولا تدخل التوكن ثم انتر.
③┘ ثانيا واخيرآ ايدي المطور ثم انتر
——————————————
⚠️║بالنسبه للسكرين ميحتاج تسوي البوت يشتغل تلقائيآ بالسكرين بدون اي مشكله.
➖➖➖➖➖ •⊱⚡️⊰• ➖➖➖➖➖
...
➖➖➖➖➖ •⊱⚡️⊰• ➖➖➖➖➖
🔕┐ كتم ~⪼ •⊱ معرف، ايدي ، رد ⊰• 
🔔┤ الغاء الكتم ~⪼ •⊱ معرف، ايدي ، رد ⊰• 
📫┤ تقيد ~⪼ •⊱ معرف، ايدي ، رد ⊰• 
📬┤ الغاء التقيد ~⪼ •⊱ معرف، ايدي ، رد ⊰•
🎭┤ حظر ~⪼ •⊱ معرف، ايدي ، رد ⊰• 
🎭┤ الغاء الحظر ~⪼ •⊱ معرف، ايدي ، رد ⊰• 
🚫┤ طرد ~⪼ •⊱ معرف، ايدي ، رد ⊰•
🌀┘ الغاء القيود~⪼ •⊱ لالغاء الكتم- التقيد- الحظر⊰• 
─ ─ ─ ─ ─ ─ ─ ─ ─ 
🔺┐ مسح المكتومين
🔻┘ مسح المحظورين
➖➖➖➖➖ •⊱⚡️⊰• ➖➖➖➖➖
...
if text:match("^7مم7$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
            if not database:get('lock:add'..msg.chat_id_) then
   local text = [[
<b>👷‍♂️╎ اهلا بك في سورس لايتنك 💯</b>
...
------------------------------------------------------------
	...
	if text == 'ارسال نسخه' and is_leader(msg) then
        if not database:get('lock:add'..msg.chat_id_) then
tdcli.sendDocument(SUDO, 0, 0, 1, nil, './Lightning.lua', dl_cb, nil)
...
-----------------------------------------------------------------------------------------
        if (idf:match("-100(%d+)") or is_momod(msg.sender_user_id_, msg.chat_id_)) and text:match("^موقعي$") or text:match("^[Mm]e$") and msg.reply_to_message_id_ == 0 and check_user_channel(msg) then
		  ...
		  local msgs = database:get("msgs:"..msg.sender_user_id_..":"..msg.chat_id_)
            if is_leaderid(result.id_) then
              Lightning4 = "SUDO BOT 🍃"
              Lightning3 = "مطور الاساسـي 🍃"
            elseif is_sudoid(result.id_) then
              Lightning4 = "SUDO 🌿"
              Lightning3 = "المطور 🌿"
            elseif is_admin(result.id_) then
              Lightning4 = "Bot Admin ✨"
              Lightning3 = "ادمن في البوت ✨"
            elseif is_owner(result.id_, msg.chat_id_) then
              Lightning4 = "Owner 🍂"
              Lightning3 = "المدير 🍂"
            elseif is_momod(result.id_, msg.chat_id_) then
              Lightning4 = "Group Admin 🎌"
              Lightning3 = "ادمن المجموعه🎌"
            elseif is_vipmem(result.id_, msg.chat_id_) then
              Lightning4 = "VIP Member ⚔️"
              Lightning3 = "عضو مميز ⚔️"
            else
              Lightning4 = "Member"
              Lightning3 = "عـضـــو 🛩️"
            end
            if result.username_ then
              username = "@" .. result.username_
            elseif database:get("lang:gp:" .. msg.chat_id_) then
...
----------------------------------------------------------------------------------------------
      ...
      if not database:get('lock:add'..msg.chat_id_) then  	
local Lightning = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
...
----------------------------------------- 
   ...
   if not database:get('lock:add'..msg.chat_id_) then     	
local Lightning = {"هلووووات  ⁽🙆‍♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
        if text == "شلونك" or text == "شلونكم" or text == "شلونج" or text == "شونج" then
        	if not database:get('lock:add'..msg.chat_id_) then
local Lightning = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
...
----------------------------------------- 
        if text == "غني" or text == "غنيلي" then
        	if not database:get('lock:add'..msg.chat_id_) then
local Lightning = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجـب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
        if text == "باي" or text == "رايح" then
        	if not database:get('lock:add'..msg.chat_id_) then
local Lightning = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
       if text == "جاو" or text == "ججاو" then
       	if not database:get('lock:add'..msg.chat_id_) then
local Lightning = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
     ...
     if not database:get('lock:add'..msg.chat_id_) then    	
local Lightning = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not database:get('lock:add'..msg.chat_id_) then local Lightning = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
  ...
  if not database:get('lock:add'..msg.chat_id_) then         	
local Lightning = {"عيـــب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
    ...
    if not database:get('lock:add'..msg.chat_id_) then     	
local Lightning = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
  ...
  if not database:get('lock:add'..msg.chat_id_) then       	
local Lightning = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
     ...
     if not database:get('lock:add'..msg.chat_id_) then    	
local Lightning = {"كافـي ضحــك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
      ...
      if not database:get('lock:add'..msg.chat_id_) then   	
local Lightning = {"لصير عصبي يرتفع ضغطـك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
        if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
        if text == "🌝" or text == "🌝🌝" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
        if text == "💋" or text == "💋💋" then
        if not database:get('lock:add'..msg.chat_id_) then 	
local Lightning = {"كبر 🙀 جان استحيتو 😹😹","عســـل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
        if text == "💔" or text == "💔💔" then
        if not database:get('lock:add'..msg.chat_id_) then 	
local Lightning = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
         if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
      if not database:get('lock:add'..msg.chat_id_) then    	
local Lightning = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
        if text == "😐" or text == "🙂" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
     ...
     if not database:get('lock:add'..msg.chat_id_) then    	
local Lightning = {"انجــب اي 😒😹","اي صحيح لا 🙊😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
...
-----------------------------------------
        if text == "ههه" or text == "هههه" or text == "ههههه" or text == "هههههه" or text == "ههههههه" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
        if text == "كيبر" or text == "Lightning" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"عيون 👀 كيبر امرني 🤠🍃","اي تفضل اغاتي🌝🍃","شرايد مني 😐💔","ها حياتي😻🍃" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
        if text == "اكلك" or text == "اكلج" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------- 
        if text == "شبيك" or text == "شبيك انت" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
---------------------------------------
if text == "فديتك" or text == "فديتج" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
--------------------------------------
if text == "ميتين" or text == "اصنام" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
------------------------------------
if text == "☺️" or text == "😊" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "اجه" or text == "اجت" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
------------------------------------------------------------
if text == "الخميس" or text == "خميس" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
--------------------------------------
if text == "ممكن نزوج" or text == "ممكن نرتبط" then
         if not database:get('lock:add'..msg.chat_id_) then 
local Lightning = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..Lightning[math.random(#Lightning)]..'', 1, 'md')
end
end
----------------------------------------------------------	   
